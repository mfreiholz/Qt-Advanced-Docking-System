...
static ADS_NS::SectionContent::RefPtr createLongTextLabelSC(
    ADS_NS::ContainerWidget* container)
{
  ...
  l->setText(QString(
      ...
      "Wörter nahm und diese durcheinander warf um ein Musterbuch zu "
      "erstellen. Es hat nicht nur 5 Jahrhunderte überlebt, sondern auch in "
      "Spruch in die elektronische Schriftbearbeitung geschafft (bemerke, "
      "nahezu unverändert). Bekannt wurde es 1960, mit dem erscheinen von "
      "Letrase, welches Passagen von Lorem Ipsum enhielt, so wie Desktop "
      "Software wie Aldus PageMaker - ebenfalls mit Lorem Ipsum."));
  bl->addWidget(l);

  const int index = ++CONTENT_COUNT;
  ADS_NS::SectionContent::RefPtr sc = ADS_NS::SectionContent::newSectionContent(
      QString("uname-%1").arg(index),
      container,
      new IconTitleWidget(QIcon(), QString("Label %1").arg(index)),
      w);
    ...
    ADS_NS::ContainerWidget* container)
{
  ...
  const int index = ++CONTENT_COUNT;
  return ADS_NS::SectionContent::newSectionContent(
      QString("uname-%1").arg(index),
      container,
      new IconTitleWidget(QIcon(), QString("Calendar %1").arg(index)),
      w);
}
    ...
    ADS_NS::ContainerWidget* container)
{
  ...
  return ADS_NS::SectionContent::newSectionContent(
      QString("uname-%1").arg(index),
      container,
      new IconTitleWidget(QIcon(), QString("Filesystem %1").arg(index)),
      w);
}
    ...
    , ui(new Ui::MainWindow)
{
  ...
  // Setup actions.
  QObject::connect(ui->actionContentList,
                   SIGNAL(triggered()),
                   this,
                   SLOT(showSectionContentListDialog()));

  // ADS - Create main container (ContainerWidget).
  _container = new ADS_NS::ContainerWidget();
#if QT_VERSION >= 0x050000
  QObject::connect(_container,
                   &ADS_NS::ContainerWidget::activeTabChanged,
                   this,
                   &MainWindow::onActiveTabChanged);
  QObject::connect(_container,
                   &ADS_NS::ContainerWidget::sectionContentVisibilityChanged,
                   this,
                   &MainWindow::onSectionContentVisibilityChanged);
#else
  QObject::connect(
      _container,
      SIGNAL(activeTabChanged(const SectionContent::RefPtr&, bool)),
      this,
      SLOT(onActiveTabChanged(const SectionContent::RefPtr&, bool)));
  QObject::connect(
      _container,
      SIGNAL(sectionContentVisibilityChanged(SectionContent::RefPtr, bool)),
      this,
      SLOT(onSectionContentVisibilityChanged(SectionContent::RefPtr, bool)));
#endif
  ...
  if (true) {
    ...
    sw = _container->addSectionContent(
        createLongTextLabelSC(cw), sw, ADS_NS::CenterDropArea);
    sw = _container->addSectionContent(
        createCalendarSC(cw), sw, ADS_NS::RightDropArea);
    sw = _container->addSectionContent(
        createFileSystemTreeSC(cw), sw, ADS_NS::CenterDropArea);
    ...
    sc->setFlags(static_cast<ADS_NS::SectionContent::Flag>(
        ADS_NS::SectionContent::AllFlags ^ ADS_NS::SectionContent::Closeable));
    _container->addSectionContent(sc);
  } else if (false) {
    // Issue #2: If the first drop is not into CenterDropArea, the application
    // crashes.
    ADS_NS::ContainerWidget* cw = _container;
    ADS_NS::SectionWidget* sw = NULL;

    sw = _container->addSectionContent(
        createLongTextLabelSC(cw), sw, ADS_NS::LeftDropArea);
    sw = _container->addSectionContent(
        createCalendarSC(cw), sw, ADS_NS::LeftDropArea);
    sw = _container->addSectionContent(
        createLongTextLabelSC(cw), sw, ADS_NS::CenterDropArea);
    sw = _container->addSectionContent(
        createLongTextLabelSC(cw), sw, ADS_NS::CenterDropArea);
    sw = _container->addSectionContent(
        createLongTextLabelSC(cw), sw, ADS_NS::CenterDropArea);
    sw = _container->addSectionContent(
        createLongTextLabelSC(cw), sw, ADS_NS::RightDropArea);
    sw = _container->addSectionContent(
        createLongTextLabelSC(cw), sw, ADS_NS::BottomDropArea);
  }
...
void MainWindow::onActiveTabChanged(const ADS_NS::SectionContent::RefPtr& sc,
                                    bool active)
{
  Q_UNUSED(active);
...
void MainWindow::onSectionContentVisibilityChanged(
