...
public:
  ...
  /*!
   * Adds the section-content <em>sc</em> to this container-widget into the
   * section-widget <em>sw</em>. If <em>sw</em> is not NULL, the <em>area</em>
   * is used to indicate how the content should be arranged. Returns a pointer
   * to the SectionWidget of the added SectionContent. Do not use it for
   * anything else than adding more SectionContent elements with this method.
   */
  SectionWidget* addSectionContent(const SectionContent::RefPtr& sc,
   ...
   * Independed of the current state, whether it is used inside a section or is
   * floating.
   */
  ...
  /*!
   * Selects the specific SectionContent as current, if it is part of a
   * SectionWidget. If SC is floating, it does nothing (or should we show it?)
   */
  ...
  /*!
   * Serializes the current state of contents and returns it as a plain byte
   * array. \see restoreState(const QByteArray&)
   */
  QByteArray saveState() const;

  /*!
   * Deserilizes the state of contents from <em>data</em>, which was written
   * with <em>saveState()</em>. \see saveState()
   */
...
private:
  ...
  SectionWidget* dropContent(const InternalContentData& data,
  ...
  SectionWidget* dropContentOuterHelper(QLayout* l,
  ...
  bool restoreFloatingWidgets(QDataStream& in,
                              int version,
                              QList<FloatingWidget*>& floatings);
  bool restoreSectionWidgets(QDataStream& in,
                             int version,
...
signals:
   ...
   * \since 0.2
   */
  void sectionContentVisibilityChanged(const SectionContent::RefPtr& sc,
                                       bool visible);
...
private:
  ...
  QPointer<QSplitter>
      _splitter;  // $mfreiholz: I'd like to remove this variable entirely,
