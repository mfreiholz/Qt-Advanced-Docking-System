...
static QPixmap createDropIndicatorPixmap(const QPalette& pal,
                                         const QSizeF& size,
                                         DropArea dropArea)
{
  ...
  const QColor areaBackgroundColor =
  ...
  if (true) {
    ...
    switch (dropArea) {
      case TopDropArea:
        ...
        break;
      case RightDropArea:
        areaRect = QRectF(baseRect.width() * .5f,
                          baseRect.y(),
                          baseRect.width() * .5f,
                          baseRect.height());
    ...
    , _lastLocation(InvalidDropArea)
{
  ...
  areaWidgets.insert(
      ...
      createDropIndicatorWidget(
          TopDropArea));  // createDropWidget(":/img/split-top.png"));
  areaWidgets.insert(
      ADS_NS::RightDropArea,
      createDropIndicatorWidget(
          RightDropArea));  // createDropWidget(":/img/split-right.png"));
  areaWidgets.insert(
      ADS_NS::BottomDropArea,
      createDropIndicatorWidget(
          BottomDropArea));  // createDropWidget(":/img/split-bottom.png"));
  areaWidgets.insert(
      ADS_NS::LeftDropArea,
      createDropIndicatorWidget(
          LeftDropArea));  // createDropWidget(":/img/split-left.png"));
  areaWidgets.insert(
      ADS_NS::CenterDropArea,
      createDropIndicatorWidget(
          CenterDropArea));  // createDropWidget(":/img/dock-center.png"));
...
{
  QPainter p(this);
  const QColor areaColor = palette().color(
      QPalette::Active, QPalette::Highlight);  // QColor(0, 100, 255)
...
void DropOverlayCross::setAreaWidgets(const QHash<DropArea, QWidget*>& widgets)
{
  ...
  while (i2.hasNext()) {
    ...
    _grid->addWidget(
        widget, opts.first.x(), opts.first.y(), (Qt::Alignment)opts.second);
  }
  reset();
}
...
DropArea DropOverlayCross::cursorLocation() const
{
  ...
  while (i.hasNext()) {
    i.next();
    if (_overlay->allowedAreas().testFlag(i.key()) && i.value()
