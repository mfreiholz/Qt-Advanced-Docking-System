...
#include "AdvancedDockingSystem/DropOverlay.h"
#include "AdvancedDockingSystem/Internal.h"
#include "AdvancedDockingSystem/SectionContentWidget.h"
#include "AdvancedDockingSystem/SectionTitleWidget.h"
#include "AdvancedDockingSystem/SectionWidget.h"
#include "AdvancedDockingSystem/Serialization.h"
...
SectionWidget* ContainerWidget::addSectionContent(
    const SectionContent::RefPtr& sc, SectionWidget* sw, DropArea area)
{
  ...
  QObject::connect(data.titleWidget,
                   &SectionTitleWidget::activeTabChanged,
                   this,
                   &ContainerWidget::onActiveTabChanged);
#else
  QObject::connect(data.titleWidget,
...
bool ContainerWidget::removeSectionContent(const SectionContent::RefPtr& sc)
{
  ...
  // Hide the content.
  // The hideSectionContent() automatically deletes no longer required
  // SectionWidget objects.
  if (!hideSectionContent(sc))
    return false;

  // Begin of ugly work arround.
  // TODO The hideSectionContent() method should take care of deleting
  // FloatingWidgets and SectionWidgets, but only cares about SectionWidgets
  // right now. So we need to check whether it was a FloatingWidget and delete
  // it.
  bool found = false;
...
bool ContainerWidget::showSectionContent(const SectionContent::RefPtr& sc)
{
  ...
  if (_hiddenSectionContents.contains(sc->uid())) {
    ...
    if (hsi.preferredSectionId > 0
        && (sw = SWLookupMapById(this).value(hsi.preferredSectionId)) != NULL)
    {
  ...
  // TODO
  qWarning(
      "Unable to show SectionContent, don't know where 8-/ (already visible?)");
  return false;
}
...
bool ContainerWidget::hideSectionContent(const SectionContent::RefPtr& sc)
{
  ...
  // separate list as long as a "showSectionContent" gets called for the SC
  // again. In case that the SW does not have any other SCs, we need to delete
  // it.
...
bool ContainerWidget::isSectionContentVisible(const SectionContent::RefPtr& sc)
{
  ...
  qWarning() << "SectionContent is not a part of this ContainerWidget:"
             << sc->uniqueName();
  return false;
}
...
QMenu* ContainerWidget::createContextMenu() const
{
  ...
  for (int i = 0; i < _sections.size(); ++i) {
    ...
    foreach(const SectionContent::RefPtr& sc, contents) {
      ...
      a->setObjectName(
          QString("ads-action-sc-%1").arg(QString::number(sc->uid())));
      ...
      QObject::connect(
          ...
          &QAction::toggled,
          this,
          &ContainerWidget::onActionToggleSectionContentVisibility);
#else
      QObject::connect(a,
                       SIGNAL(toggled(bool)),
                       this,
                       SLOT(onActionToggleSectionContentVisibility(bool)));
#endif
...
SectionWidget* ContainerWidget::dropContent(const InternalContentData& data,
                                            SectionWidget* targetSection,
                                            DropArea area,
                                            bool autoActive)
{
...
SectionWidget* ContainerWidget::dropContentOuterHelper(
    QLayout* l,
    const InternalContentData& data,
...
QByteArray ContainerWidget::saveHierarchy() const
{
  ...
  */
  ...
  if (_mainLayout->count() <= 0 || _sections.isEmpty()) {
    ...
    while (iter.hasNext()) {
      iter.next();
      if (iter.value().preferredSectionId <= 0
          || !SWLookupMapById(this).contains(iter.value().preferredSectionId))
        cnt++;
    }
    out << cnt;
    iter.toFront();
    ...
    out << -1;
    qWarning() << "Oh noooz.. Something went wrong. There are too many items "
                  "in _mainLayout.";
  }
  return ba;
}
...
void ContainerWidget::saveSectionWidgets(QDataStream& out,
...
bool ContainerWidget::saveSectionIndex(ADS_NS_SER::SectionIndexData& sid) const
{
  ...
  for (int i = 0; i < sid.sectionsCount; ++i) {
    ...
    se.x = mapFromGlobal(
               _sections[i]->parentWidget()->mapToGlobal(_sections[i]->pos()))
               .x();
    se.y = mapFromGlobal(
               _sections[i]->parentWidget()->mapToGlobal(_sections[i]->pos()))
               .y();
...
bool ContainerWidget::restoreHierarchy(const QByteArray& data)
{
  ...
  if (mode == 0) {
    ...
    success =
        restoreSectionWidgets(in, version, NULL, sections, contentsToHide);
    if (!success)
    ...
    for (int i = 0; i < cnt; ++i) {
      ...
      if (!takeContent(sc, data)) {
        qWarning(
            "This should never happen! Looks like a bug during serialization, "
            "since the content is already being used in SWs.");
        continue;
      }
      ...
      if (sections.size() <= 0)
        qFatal(
            "This should never happen, because above a section should have "
            "been created.");
      else
        sw = sections.first();
  ...
  // What shall we do with it? For now: Simply drop them into the first
  // SectionWidget.
  if (true) {
    ...
    const QList<SectionContent::WeakPtr> allContents =
        SCLookupMapById(this).values();
...
bool ContainerWidget::restoreFloatingWidgets(QDataStream& in,
                                             int version,
                                             QList<FloatingWidget*>& floatings)
{
  Q_UNUSED(version)
  ...
  for (int i = 0; i < fwCount; ++i) {
    ...
    const SectionContent::RefPtr sc =
        SCLookupMapByName(this).value(uname).toStrongRef();
    if (!sc) {
    ...
    FloatingWidget* fw = new FloatingWidget(
        this, sc, data.titleWidget, data.contentWidget, this);
    ...
    floatings.append(fw);
    data.titleWidget->_fw =
        fw;  // $mfreiholz: Don't look at it :-< It's more than ugly...
  }
  return true;
}

bool ContainerWidget::restoreSectionWidgets(
    QDataStream& in,
    int version,
    ...
    QList<SectionContent::RefPtr>& contentsToHide)
{
  ...
  else if (type == 2)
  {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      InternalContentData data;
      if (!takeContent(sc, data)) {
        qCritical() << "Can not find InternalContentData of SC, this should "
                       "never happen!"
                    << sc->uid() << sc->uniqueName();
        sw->addContent(sc);
      } else
        sw->addContent(data, false);
...
bool ContainerWidget::takeContent(const SectionContent::RefPtr& sc,
                                  InternalContentData& data)
{
  ADS_Expects(!sc.isNull());
...
void ContainerWidget::onActionToggleSectionContentVisibility(bool visible)
{
  ...
  const SectionContent::RefPtr sc =
      SCLookupMapById(this).value(uid).toStrongRef();
  if (sc.isNull()) {
