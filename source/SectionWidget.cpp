...
#include "AdvancedDockingSystem/FloatingWidget.h"
...
#else
  // QObject::connect(_tabsMenuButton, SIGNAL(clicked()), this,
  // SLOT(onTabsMenuButtonClicked()));
#endif
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
  QObject::connect(_closeButton,
                   &QPushButton::clicked,
                   this,
                   &SectionWidget::onCloseButtonClicked);
#else
  QObject::connect(
      _closeButton, SIGNAL(clicked(bool)), this, SLOT(onCloseButtonClicked()));
#endif
...
SectionWidget::~SectionWidget()
{
  if (_container) {
    ...
    _container->_sections.removeAll(this);  // Note: I don't like this here, but
                                            // we have to remove it from list...
  }
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
  QObject::connect(title,
                   &SectionTitleWidget::clicked,
                   this,
                   &SectionWidget::onSectionTitleClicked);
#else
  QObject::connect(
      title, SIGNAL(clicked()), this, SLOT(onSectionTitleClicked()));
#endif
...
void SectionWidget::addContent(const InternalContentData& data,
                               bool autoActivate)
{
  ...
  _tabsLayout->insertWidget(_tabsLayout->count() - _tabsLayoutInitCount,
                            data.titleWidget);
...
#else
  QObject::connect(
      data.titleWidget, SIGNAL(clicked()), this, SLOT(onSectionTitleClicked()));
#endif
  ...
  // Mark it as inactive tab.
    ...
    data.titleWidget->setActiveTab(
        false);  // or: setCurrentIndex(currentIndex())
...
#if QT_VERSION < QT_VERSION_CHECK(5, 0, 0)
    title->setAttribute(Qt::WA_WState_Created,
                        false); /* fix: floating rubberband #16 */
#endif
    title->disconnect(this);
    title->setParent(_container);
#if QT_VERSION < QT_VERSION_CHECK(5, 0, 0)
    title->setAttribute(Qt::WA_WState_Created,
                        true); /* fix: floating rubberband #16 */
#endif
  }
...
int SectionWidget::indexOfContentByTitlePos(const QPoint& p,
                                            QWidget* exclude) const
{
  int index = -1;
  for (int i = 0; i < _sectionTitles.size(); ++i) {
    if (_sectionTitles[i]->geometry().contains(p)
        && (exclude == NULL || _sectionTitles[i] != exclude))
    {
      index = i;
      break;
    }
...
{
  if (from >= _contents.size() || from < 0 || to >= _contents.size() || to < 0
      || from == to)
  {
...
void SectionWidget::setCurrentIndex(int index)
{
  ...
  for (int i = 0; i < _tabsLayout->count(); ++i) {
    ...
    if (item->widget()) {
      SectionTitleWidget* stw =
          dynamic_cast<SectionTitleWidget*>(item->widget());
      if (stw) {
        if (i == index) {
...
void SectionWidget::updateTabsMenu()
{
  ...
  for (int i = 0; i < _contents.count(); ++i) {
    ...
    QObject::connect(a,
                     &QAction::triggered,
                     this,
                     &SectionWidget::onTabsMenuActionTriggered);
#else
    QObject::connect(a,
                     SIGNAL(triggered(bool)),
                     this,
                     SLOT(onTabsMenuActionTriggered(bool)));
#endif
  }
