...
SectionTitleWidget::SectionTitleWidget(SectionContent::RefPtr content,
                                       QWidget* parent)
    : QFrame(parent)
    , _content(content)
    , _tabMoving(false)
...
#  endif
        ...
        QPropertyAnimation* resizeAnim =
            new QPropertyAnimation(_fw, "size", this);
    ...
    else
    {
      ...
      if (cw->outerBottomDropRect().contains(
              cw->mapFromGlobal(ev->globalPos())))
...
void SectionTitleWidget::mouseMoveEvent(QMouseEvent* ev)
{
  ...
  if (_fw && (ev->buttons() & Qt::LeftButton)) {
    ...
    const QPoint moveToPos = ev->globalPos()
        - (_dragStartPos
           + QPoint(ADS_WINDOW_FRAME_BORDER_WIDTH,
  ...
  // Begin to drag/float the SectionContent.
           ...
           && !section->titleAreaGeometry().contains(
               section->mapFromGlobal(ev->globalPos())))
  {
    ...
    if (!section->takeContent(_content->uid(), data)) {
      qWarning() << "THIS SHOULD NOT HAPPEN!!" << _content->uid()
                 << _content->uniqueName();
      return;
    }

    _fw = new FloatingWidget(
        cw, data.content, data.titleWidget, data.contentWidget, cw);
  ...
  // Begin to drag title inside the title area to switch its position inside the
  // SectionWidget.
  else if (!_dragStartPos.isNull() && (ev->buttons() & Qt::LeftButton)
           && (ev->pos() - _dragStartPos).manhattanLength()
               >= QApplication::startDragDistance()  // Wait a few pixels before
                                                     // start moving
           && (section = findParentSectionWidget(this)) != NULL
           && section->titleAreaGeometry().contains(
               section->mapFromGlobal(ev->globalPos())))
